...
class CommandLineWPS(Process):
    def __init__(
            self, toolpath_object: CommentedMap, loadingContext: LoadingContext
    ) -> None:
        """Initialize this CommandLineWPS."""
        super(CommandLineWPS, self).__init__(toolpath_object, loadingContext)
        self.prov_obj = loadingContext.prov_obj
        self.tool["baseCommand"]='/tmp/wps.sh'
    ...
    def make_job_runner(self, runtimeContext: RuntimeContext) -> Type[JobBase]:
        ...
        if dockerRequired:
            raise UnsupportedRequirement(
                "--no-container, but this CommandLineWPS has "
                "DockerRequirement under 'requirements'."
            )
        return CommandLineJob
